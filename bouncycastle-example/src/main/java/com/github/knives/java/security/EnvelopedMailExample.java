...
public class EnvelopedMailExample {
	public static void main(String args[]) throws Exception {
		...
		MimeBodyPart envPart = gen.generate(dataPart,
				SMIMEEnvelopedGenerator.AES256_CBC, "BC");

		// create the mail message
		MimeMessage mail = Utils.createMimeMessage("example enveloped message",
				envPart.getContent(), envPart.getContentType());
		...
		if (recipient != null) {
			// decryption step
			MimeBodyPart recoveredPart = SMIMEUtil.toMimeBodyPart(recipient
					.getContent(key, "BC"));
