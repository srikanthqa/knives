 ...
 */
public class EnvelopedSignedMailExample extends SignedDataProcessor {
	public static void main(String[] args) throws Exception {
		...
		MimeMultipart signedMultipart = SignedMailExample
				.createMultipartWithSignature(key, cert, certsAndCRLs, dataPart);
		...
		MimeBodyPart envPart = gen.generate(signedPart,
				SMIMEEnvelopedGenerator.AES256_CBC, "BC");

		// create the mail message
		MimeMessage mail = Utils.createMimeMessage(
				"example signed and enveloped message", envPart.getContent(),
		...
		// decryption step
		MimeBodyPart res = SMIMEUtil.toMimeBodyPart(recipient.getContent(key,
				"BC"));
