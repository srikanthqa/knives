 ...
 */
public class SignedMailExample extends SignedDataProcessor {
	public static MimeMultipart createMultipartWithSignature(PrivateKey key,
			X509Certificate cert, CertStore certsAndCRLs, MimeBodyPart dataPart)
			throws Exception {
		...
		signedAttrs.add(new SMIMEEncryptionKeyPreferenceAttribute(SMIMEUtil
				.createIssuerAndSerialNumberFor(cert)));
	...
	public static void main(String args[]) throws Exception {
		...
		// create the signed message
		MimeMultipart multiPart = createMultipartWithSignature(key, cert,
				certsAndCRLs, dataPart);

		// create the mail message
		MimeMessage mail = Utils.createMimeMessage("example signed message",
